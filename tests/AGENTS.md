...
- Process-graph suites: `app-shell.test.ts`, `window-graph.test.ts` (incl. hide coalesce + utility foreground), `secure-web-preferences.test.ts`, `utility-presentation.test.ts`, `composition-wiring.test.ts`, `composition-root.test.ts`.
...
- ~55 test files / 600+ tests (Vitest workspace).
