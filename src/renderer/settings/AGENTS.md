...
- Identity strip: 48px hero icon in `.icon-aurora-stage` with shared coffee-brown aurora (`../styles/icon-aurora.css` — ring/halo/blobs/sparks); tighter local blur for compact scale.
...
- Respect `prefers-reduced-motion`, `prefers-reduced-transparency`, and `prefers-contrast` for controls, group materials, and the icon aurora.
