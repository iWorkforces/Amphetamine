...
- Windows job packages x64 on `windows-latest` (NSIS + portable `.exe`).
- Artifacts: `dist-mac-{arch}` (dmg/zip) and `dist-win-x64` (exe/yml/blockmap) for 14 days (no `-beta` suffix).
...
- It downloads `dist-mac-arm64`, `dist-mac-x64`, and `dist-win-x64` artifacts from that CI run.
...
- Packaging: mac arm64/x64 plus Windows x64 (`package` + `package-windows` jobs).
- After `electron-builder`, basenames get a `-beta` suffix (e.g. `Amphetamine-1.9.5-arm64-beta.dmg`, `*-beta.exe`).
- Artifacts: `dist-mac-beta-{arch}` and `dist-win-beta-x64` for 14 days.
